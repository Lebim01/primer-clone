...
const Select = ({ style: { width = "w-full" }, options = [], ...props }: Props) => {
  ...
  return (
    <div ref={ref} className={`relative ${width}`}>
      <div className={`text-selection-none flex w-full items-center rounded border p-2 text-sm hover:cursor-pointer hover:bg-hover-card`} onClick={() => setOpen(s => !s)}>
      ...
      {open &&
        <div className="absolute -bottom-1 z-10 flex w-full translate-y-full flex-col gap-1 rounded border bg-white shadow-md">
          {options.map((opt) => (
